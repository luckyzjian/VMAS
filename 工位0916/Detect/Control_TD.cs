    ...
    public partial class Control_TD : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                Msg(Msg_gl, panel_gl, CarWait.igbt.Power.ToString());
                Msg(Msg_jsd, panel_jsd, CarWait.igbt.Acceleration);
                Msg(Msg_xsjl, panel_xsjl, CarWait.igbt.Range);
        ...
        private void button_ks_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    CarWait.igbt.Exit_Control();
                    CarWait.igbt.Set_Control_Power(float.Parse(textBox_jzl.Text.Trim()));
                    CarWait.igbt.Start_Control_Power();
                }
                else if (radioButton2.Checked == true)
                {
                    CarWait.igbt.Exit_Control();
                    CarWait.igbt.Set_Control_Force(int.Parse(textBox_jzl.Text.Trim()));
                    CarWait.igbt.Start_Control_Force();
                }
                else if (radioButton3.Checked == true)
                {
                    CarWait.igbt.Exit_Control();
                    CarWait.igbt.Set_Speed(float.Parse(textBox_jzl.Text.Trim()));
                    CarWait.igbt.Start_Control_Speed();
                }
            }
            catch (Exception)
            {

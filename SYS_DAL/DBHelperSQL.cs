    ...
    public class DBHelperSQL
    {
        ...
        public static string Init_DBlink()
        {
            ...
            try
            {
                ...
                temp.Length = 2048;
                ini.INIIO.GetPrivateProfileString("数据", "服务器", "", temp, 2048, @".\Config.ini");
                string ServerName = temp.ToString().Trim();
                ini.INIIO.GetPrivateProfileString("数据", "数据库", "", temp, 2048, @".\Config.ini");
                string DBName = temp.ToString().Trim();
                ini.INIIO.GetPrivateProfileString("数据", "用户名", "", temp, 2048, @".\Config.ini");
                string UserName = temp.ToString().Trim();
                ini.INIIO.GetPrivateProfileString("数据", "密码", "", temp, 2048, @".\Config.ini");
        ...
        public static void DB_Link_Test_DAL()
        {
                using (SqlConnection conn_test = new SqlConnection(constr))
                {
                    try
                    {
                        ...
                        cmd.CommandText = "select * from JCXZT";        //查询这张表 如果没有异常则连接正常
        ...
        public static bool DB_Link_Test()
        {
            ...
            {
                SqlCommand cmd=conn_test.CreateCommand();
                cmd.CommandText = "select * from JCXZT";
                test_adp = new SqlDataAdapter(cmd);
                test_adp.Fill(test_dt);
                return true;
            }

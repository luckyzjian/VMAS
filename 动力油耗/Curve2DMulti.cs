...
{
    public class DrawingCurveMulti
    {
        /// <summary>
        /// 画曲线图
        /// </summary>
        ...
        {
            Curve2DMulti cuv2D = new Curve2DMulti();
            cuv2D.Fit();
            return cuv2D.CreateImage();
        }
    }
    public class Curve2DMulti
    {
        ...
        private float fltYSliceValue2 = 200;//Y轴右侧刻度的数值宽度
        ...
        private string strYAxisText2 = "万元"; //Y轴说明文字
        ...
        private float[] fltsValues2 = new float[] { 20.0f, 30.0f, 50.0f, 55.4f, 21.6f, 12.8f, 99.5f, 36.4f, 78.2f, 56.4f, 45.8f, 66.5f, 99.5f, 36.4f, 78.2f, 56.4f, 45.8f, 66.5f, 20.0f, 30.0f, 50.0f, 55.4f, 21.6f, 12.8f }; //值
        ...
        public Curve2DMulti()
        {
            ...
            scale1Value = 0;
            scale2Value = 0;
            scale1Color = Color.Yellow;
            scale2Color = Color.Yellow;
        }
        public bool displayScale1 { set; get; }
        public bool displayScale2 { set; get; }
        public float scale1Value { set; get; }
        public float scale2Value { set; get; }
        public Color scale1Color { set; get; }
        public Color scale2Color { set; get; }
        /// <summary>
        /// 图像的宽度
        /// </summary>
        public float Width
        ...
        /// Y2轴最大值 
        /// </summary>
        public float YMaxValue2
        {
            set { yMaxValue2 = value; }
            get { return yMaxValue2; }
        }
        /// <summary>
        /// X轴刻度宽度
        /// </summary>
        ...
        /// Y2轴刻度的数值宽度
        /// </summary>
        public float YSliceValue2
        {
            set { fltYSliceValue2 = value; }
            get { return fltYSliceValue2; }
        }
        /// <summary>
        /// Y轴刻度开始值
        /// </summary>
        ...
        {
            set { fltsValues2 = value; }
            get { return fltsValues2; }
        }
        ...
        /// <summary>
         /// Y2轴说明文字
         /// </summary>
        public string YAxisText2
        {
            set { strYAxisText2 = value; }
            get { return strYAxisText2; }
        }
        ...
        public void Fit()
        {
            ...
            if (displayScale1)
            {
                if (fltMinValue > scale1Value)
                    fltMinValue = scale1Value;
                else if (fltMaxValue < scale1Value)
                    fltMaxValue = scale1Value;
            }
            if (displayScale2)
            {
                if (fltMinValue > scale2Value)
                    fltMinValue = scale2Value;
                else if (fltMaxValue < scale2Value)
                    fltMaxValue = scale2Value;
            }
            if (fltMinValue < 0)
                fltMinValue = 0;
            ...
            int intYSliceCount = (int)((fltMaxValue-fltMinValue) / YSliceValue);
            if (fltMaxValue % YSliceValue != 0 || intYSliceCount == 0)
            {
                intYSliceCount++;
            }
        ...
        public Bitmap CreateImage()
        {
            ...
            if (intKeysCount * 10 > intValuesCount)
            {
                float[] fltCurrentValues = new float[intValuesCount];
                float[] fltCurrentValues2 = new float[intValuesCount];
                for (int j = 0; j < intValuesCount; j++)
                {
                    fltCurrentValues[j] = Values[j];
                    fltCurrentValues2[j] = Values2[j];
                }
                DrawContent(ref objGraphics, fltCurrentValues, fltCurrentValues2, clrsCurveColors[0], clrsCurveColors[1]);
            }
            else
            {
                float[] fltCurrentValues = new float[intKeysCount * 10];
                float[] fltCurrentValues2 = new float[intKeysCount * 10];
                for (int j = 0; j < intKeysCount * 10; j++)
                {
                    fltCurrentValues[j] = Values[j];
                    fltCurrentValues2[j] = Values[j];
                }
                DrawContent(ref objGraphics, fltCurrentValues, fltCurrentValues2,clrsCurveColors[0], clrsCurveColors[1]);
            }
        ...
        private void InitializeGraph()
        {
            ...
            {
                fltX1 = XSpace;
                fltY1 = (Height - YSpace) + YSliceBegin - YSlice * (scale1Value / YSliceValue);
                fltX2 = Width - XSpace + XSlice / 2;
                fltY2 = fltY1;
                objGraphics.DrawLine(new Pen(new SolidBrush(scale1Color), 3), fltX1, fltY1, fltX2, fltY2);
            }
            if (displayScale2)
            {
                fltX1 = XSpace;
                fltY1 = (Height - YSpace) + YSliceBegin - YSlice * (scale2Value / YSliceValue);
                fltX2 = Width - XSpace + XSlice / 2;
                fltY2 = fltY1;
                objGraphics.DrawLine(new Pen(new SolidBrush(scale2Color),3), fltX1, fltY1, fltX2, fltY2);
            }
            //画Y轴
            fltX1 = XSpace;
            fltY1 = Height - YSpace;
        ...
        private void SetAxisText(ref Graphics objGraphics)
        {
            ...
            objGraphics.DrawString(YAxisText2, new Font("宋体", FontSize), new SolidBrush(AxisTextColor), Width-XSpace-FontSize-5, fltY);
        ...
        private void SetXAxis(ref Graphics objGraphics)
        {
            ...
            int iSliceCount = 1;
            float Scale = 0;
            float iWidth = ((Width - 2 * XSpace) / XSlice) * 100; //将要画刻度的长度分段，并乘以50，以10为单位画刻度线。
            ...
            {
                Scale = i * XSlice / 100;//即(i / 10) * (XSlice / 5)，将每个刻度分五部分画，但因为i以10为单位，得除以10
                ...
                else
                {
                    ...
                    //objGraphics.DrawLine(new Pen(new SolidBrush(SliceColor)), Width-XSpace + Scale, fltY1 + fltSliceHeight, Width - XSpace + Scale, fltY2 - fltSliceHeight);
                }
                iCount++;
            }
        }
        ...
        /// </summary>
        /// <param name="objGraphics"></param>
        private void SetYAxis2(ref Graphics objGraphics)
        {
            float fltX1 = Width-XSpace;
            float fltY1 = Height - YSpace;
            float fltX2 = Width - XSpace;
            float fltY2 = Height - YSpace;
            int iCount = 0;
            ...
            for (int i = 0; i < iHeight; i += 10)
            {
                ...
                if (iCount == 5)
                {
                    ...
                    */
                    //这里显示Y轴刻度
                    strSliceText = Convert.ToString(fltYSliceValue2 * iSliceCount + YSliceBegin);
                    objGraphics.TranslateTransform(Width-XSpace, fltY1 - Scale); //平移图像(原点)
                    objGraphics.RotateTransform(YRotateAngle, MatrixOrder.Prepend); //旋转图像
                    objGraphics.DrawString(strSliceText, new Font("宋体", FontSize), new SolidBrush(SliceTextColor), 0, 0);
                    objGraphics.ResetTransform(); //重置图像
        ...
        /// </summary>
        /// <param name="objGraphics"></param>
        private void DrawContent(ref Graphics objGraphics, float[] fltCurrentValues,float[] fltCurrentValues2, Color clrCurrentColor, Color clrCurrentColor2)
        {
            Pen CurvePen = new Pen(clrCurrentColor, CurveSize);
            Pen CurvePen2 = new Pen(clrCurrentColor2, CurveSize);
            PointF[] CurvePointF = new PointF[fltCurrentValues.Length];
            PointF[] CurvePointF2 = new PointF[fltCurrentValues.Length];
            float keys = 0;
            float values = 0;
            float yvaluemax = YMaxValue;
            float yvaluemax2 = YMaxValue * (YSliceValue2 / YSliceValue);//Y2轴的最大值通过Y1的最大值与Y2的刻度值 来计算
            for (int i = 0; i < fltCurrentValues.Length; i++)
            {
                keys = XSlice * i / 10 + XSpace;

                if (fltCurrentValues[i] < 0) fltCurrentValues[i] = 0;
                else if(fltCurrentValues[i] >yvaluemax) fltCurrentValues[i] = yvaluemax;
                values = (Height - YSpace) + YSliceBegin - YSlice * (fltCurrentValues[i] / YSliceValue);
                CurvePointF[i] = new PointF(keys, values);

                if (fltCurrentValues2[i] < 0) fltCurrentValues2[i] = 0;
                else if (fltCurrentValues2[i] > yvaluemax2) fltCurrentValues2[i] = yvaluemax2;
                values = (Height - YSpace) + YSliceBegin - YSlice * (fltCurrentValues2[i] / YSliceValue2);
                CurvePointF2[i] = new PointF(keys, values);
            }
            if (CurvePointF.Length > 1)
            {
                objGraphics.DrawCurve(CurvePen, CurvePointF, Tension);
                objGraphics.DrawCurve(CurvePen2, CurvePointF2, Tension);
            }
        }
        ...
        {
            objGraphics.DrawString(Title, new Font("宋体", FontSize), new SolidBrush(TextColor), new Point((int)(Width/2) - (intFontSize * Title.Length)/2, (int)(YSpace - YSlice / 2 - intFontSpace)));
        }
    }
}

    ...
    public partial class Main : Form
    {
        ...
        public void Init()
        {
            ...
            try
            {
                ...
                Init_Comm(mainPanelForm.jcxxxb.DPCGJPZ.Split(new string[] { "||" }, StringSplitOptions.RemoveEmptyEntries)[0],mainPanelForm.jcxxxb.DPCGJPZ.Split(new string[] { "||" }, StringSplitOptions.RemoveEmptyEntries)[1]);                           //初始化串口
                                                                                                     //实时刷新数据
            }
            catch (Exception e)
            {
                    ...
                    try
                    {
                        ...
                        if (All_Content_byte.Count > 0)
                        {
                            ...
                            if (force_list.Count >=40)
                            {
                                ...
                                {
                                    Speed_string = (speed_sum/40f).ToString("0.0");
       ...
       /*private void button_demarcate_force_Click(object sender, EventArgs e)
        ...
        {
            Exit_Control();//退出所有控制
            _continue = false;//使resolve进程不再进行
            Thread.Sleep(5);//等待进程执行完毕
            readThread.Abort();//退出进程
            ComPort_2.Close();//关闭串口
            this.Close();//关闭面板
        }

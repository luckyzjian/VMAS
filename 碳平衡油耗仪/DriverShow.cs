...
namespace 碳平衡油耗仪
{
    public partial class DriverShow : Form
    {
        ...
        {
            if (!yhTest.isReloadData)
            {
                curve1.scale1Value = yhTest.curvescalevalue;
                curve1.scale1Color = Color.Red;
                curve1.displayScale1 = yhTest.displayCurveScale;
                curve1.Values = yhTest.speedlist.ToArray();
                curve1.Values2 = yhTest.forcelist.ToArray();
                curve1.Fit();
                pictureBox1.Image = curve1.CreateImage();
            }
        }

...
namespace 速度标定
{
    public partial class Main : Form
    {
        ...
        {
            if (dt_speed.Rows.Count > 0)
            {
                csvwriter.SaveCSV(dt_speed, "C:/jcdatatxt/" + "SpeedCal.csv");
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridViewSpeed.SelectedRows.Count > 0)
                {
                    for (int i = 0; i < dataGridViewSpeed.SelectedRows.Count; i++)
                        foreach (DataRow dr in dt_speed.Rows)
                        {
                            if (dr["标定速度点(Km/h)"].ToString() == dataGridViewSpeed.SelectedRows[i].Cells["标定速度点(Km/h)"].Value.ToString())
                                drlist.Add(dr);
                        }
                    if (drlist.Count != 0)
            ...
            if (Test_Flag == false)
            {
                if (th_speedDermarcate == false)//如果未执行
                {
                    ...
                    else if (radioButtonSerial.Checked == true)
                    {
                        ...
                        if (bpq != null)
                        {
                            double control_speed = float.Parse(textBoxMubiaoSpeed.Text.Trim());
                            Motor_Open(control_speed * configdata.BpqXs);
                        }
                        buttonStartSpeed.Text = "停止";
                        groupBox12.Enabled = false;
                    }
        ...
        private void buttonAddSpeed_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dr["判定"] = (Math.Abs(double.Parse(textBoxRealSpeed.Text.Trim()) - double.Parse(textBoxStandardSpeed.Text.Trim())) > 0.2) ? "不合格" : "合格";
